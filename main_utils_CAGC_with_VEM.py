...
def setup_training_loop_kwargs():
    ...
    kwargs = ['outdir', 'num_gpus', 'snap', 'metrics', 'random_seed',
              ...
              'teacher_ckpt', 'student_ckpt', 'sparse_loss', 'sparsity_eta', 'kd_percept_mode', 'kd_percept_lambda', 'kd_l1_mode', 'kd_l1_lambda', 'pruning_ratio', 'kd_VID', 'kd_VID_langevin_steps', 'kd_VID_step_size', 'kd_VID_sigma_t_eta', 'kd_VID_MI_lambda', 'kd_VID_l2_lambda'
              ]
    ...
    # ---------------------------------------------------
    # Content Aware GAN Compression with VEM settings: teacker_ckpt, student_ckpt, kd_percept_mode, kd_percept_lambda, kd_l1_mode, kd_l1_lambda, pruning_ratio
    # ---------------------------------------------------
    ...
    # Variational EBM
    if args.kd_VID is None:
        args.kd_VID = False
    assert isinstance(args.kd_VID, bool)
    
    args.VEM_kwargs = None
    args.VEM_opt_kwargs = None
    if args.kd_VID:       
        args.VEM_kwargs = dnnlib.EasyDict(class_name='training.networks_CAGC_with_VEM.ResnetEBM', input_nc=64, n_blocks=7)
        args.VEM_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', lr=0.0001, betas=(0.0,0.999), eps=1e-8)
    
    if args.kd_VID_langevin_steps is None:
        args.kd_VID_langevin_steps = 10
    assert isinstance(args.kd_VID_langevin_steps, int)
    
    if args.kd_VID_step_size is None:
        args.kd_VID_step_size = 50.0
    assert isinstance(args.kd_VID_step_size, float)
    
    if args.kd_VID_sigma_t is None:
        args.kd_VID_sigma_t = 0.005
    assert isinstance(args.kd_VID_sigma_t, float) 
    
    if args.kd_VID_MI_lambda is None:
        args.kd_VID_MI_lambda = 0.0
    assert isinstance(args.kd_VID_MI_lambda, float)
    
    if args.kd_VID_l2_lambda is None:
        args.kd_VID_l2_lambda = 0.0
    assert isinstance(args.kd_VID_l2_lambda, float)
    ...
    # Set Content Aware GAN Compression Loss kwargs
    args.loss_kwargs.class_name = 'training.loss_CAGC_with_VEM.StyleGAN2Loss'
    ...
    args.loss_kwargs.kd_VID_langevin_steps = args.kd_VID_langevin_steps
    args.loss_kwargs.kd_VID_step_size = args.kd_VID_step_size
    args.loss_kwargs.kd_VID_sigma_t = args.kd_VID_sigma_t    
    args.loss_kwargs.kd_VID_MI_lambda = args.kd_VID_MI_lambda
    args.loss_kwargs.kd_VID_l2_lambda = args.kd_VID_l2_lambda
    
    # When finetune pruned network
    args.G_kwargs.class_name = 'training.networks_CAGC_with_VEM.Generator'
