...
def get_config_args():
    ...
    parser.add_argument(
        "-n", "--dry_run", help="Print training options and exit", action="store_true"
    )
...
def setup_training_loop_kwargs():
    ...
    kwargs = ["outdir", "num_gpus", "snap", "metrics", "random_seed",
              ...
              "resume", "freezed", "resume_train", 
              "fp32", "nhwc", "allow_tf32", "nobench", "workers",
              "teacher_ckpt", "student_ckpt", "kd_content_lambda", "kd_gram_lambda", "kd_l1_lambda", "online_distillation", "kd_res_generator", "kd_res_discriminator", "pruning_ratio",]
    ...
    args.D_kwargs = dnnlib.EasyDict(class_name='training.networks_GCC.Discriminator', block_kwargs=dnnlib.EasyDict(), mapping_kwargs=dnnlib.EasyDict(), epilogue_kwargs=dnnlib.EasyDict())
    ...
    if args.aug != "noaug":
        args.augment_kwargs = dnnlib.EasyDict(
            class_name="training.augment.AugmentPipe", **augpipe_specs[args.augpipe]
        )
    ...
    # Continue train
    if args.resume_train is not None or isinstance(args.resume_train, str):
        args.resume_train_pkl = args.resume_train
        with dnnlib.util.open_url(args.resume_train) as f:
            import legacy

            resume_data = legacy.load_last_pkl(f)
            _dir = os.path.dirname(args.resume_train)
    ...
    # ---------------------------------------------------
    # Generator-Discriminator Cooperative Compression settings: teacker_ckpt, student_ckpt, kd_content_lambda, kd_gram_lambda, kd_l1_lambda, online_distillation
    # ---------------------------------------------------
    ...
    if args.kd_content_lambda is None:
        args.kd_content_lambda = 0.0
    args.kd_content_lambda = float(args.kd_content_lambda)
    assert isinstance(args.kd_content_lambda, float)

    if args.kd_gram_lambda is None:
        args.kd_gram_lambda = 0.0
    args.kd_gram_lambda = float(args.kd_gram_lambda)
    assert isinstance(args.kd_gram_lambda, float)
    ...
    if args.online_distillation is None:
        args.online_distillation = True
    assert isinstance(args.online_distillation, bool)

    if args.kd_res_generator is None:
        args.kd_res_generator = []
    assert isinstance(args.kd_res_generator, list)
    
    if args.kd_res_discriminator is None:
        args.kd_res_discriminator = []
    assert isinstance(args.kd_res_discriminator, list)
    
    args.kd_res = dnnlib.EasyDict(generator=args.kd_res_generator, discriminator=args.kd_res_discriminator)
    ...
    # Set Generator-Discriminator Cooperative Compression Loss kwargs
    args.loss_kwargs.class_name = "training.loss_GCC.StyleGAN2Loss"
    args.loss_kwargs.kd_content_lambda = args.kd_content_lambda
    args.loss_kwargs.kd_gram_lambda = args.kd_gram_lambda
    args.loss_kwargs.kd_l1_lambda = args.kd_l1_lambda
    args.loss_kwargs.kd_res = args.kd_res
    args.loss_kwargs.online_distillation = args.online_distillation

    # When finetune pruned network
    args.G_kwargs.class_name = "training.networks_GCC.Generator"
    args.D_kwargs.class_name = "training.networks_GCC.Discriminator"
    args.G_teacher_kwargs = args.G_kwargs.copy()
    args.G_teacher_kwargs['pruning_ratio'] = 0.0
    args.G_teacher_kwargs['is_teacher'] = True
    args.G_student_kwargs = args.G_kwargs.copy()
    args.G_student_kwargs['pruning_ratio'] = args.pruning_ratio
    args.G_student_kwargs['is_teacher'] = False
    args.D_teacher_kwargs = args.D_kwargs.copy()
    args.D_student_kwargs = args.D_kwargs.copy()
    del args.G_kwargs
    del args.D_kwargs
