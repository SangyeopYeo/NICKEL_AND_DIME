...
class StyleGAN2Loss(Loss):
    def __init__(self, device, img_resolution, G_mapping, G_synthesis, D, G_teacher_mapping, G_teacher_synthesis, augment_pipe=None, style_mixing_prob=0.9, r1_gamma=10, pl_batch_shrink=2, pl_decay=0.01, pl_weight=2, kd_percept_mode='LPIPS', kd_percept_lambda=3.0, kd_l1_mode='Output_Only', kd_l1_lambda=0.0):
        super().__init__()
        self.device = device
    ...
    def run_G(self, z, c, sync, SKD_return = False, do_Gmain=False):
        ...
        with misc.ddp_sync(self.G_synthesis, sync):
            ...
            if SKD_return:
                if self.return_rgb_list:
                    img, _, student_rgb_img_list, student_SKD_feat_list = self.G_synthesis(ws, False, self.return_rgb_list, SKD_return = True)
                else:
                    img, student_SKD_feat_list = self.G_synthesis(ws, SKD_return = True)
            else:
                if self.return_rgb_list:
        ...
        with misc.ddp_sync(self.G_teacher_synthesis, sync):
            ...
            if SKD_return:
                if self.return_rgb_list:
                    img_teacher, _, teacher_rgb_img_list, teacher_SKD_feat_list = self.G_teacher_synthesis(ws_teacher, False, self.return_rgb_list, SKD_return = True)
                else:
                    img_teacher, teacher_SKD_feat_list = self.G_teacher_synthesis(ws_teacher, SKD_return = True)
            else:
                if self.return_rgb_list:
        ...
        if SKD_return:
            return img, ws, img_teacher, ws_teacher, styles_list, student_rgb_img_list, teacher_rgb_img_list, student_SKD_feat_list, teacher_SKD_feat_list
        else:
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws, gen_teacher_img, _gen_teacher_ws, styles_list, student_rgb_img_list, teacher_rgb_img_list, student_SKD_feat_list, teacher_SKD_feat_list = self.run_G(gen_z, gen_c, sync=(sync and not do_Gpl), SKD_return = True, do_Gmain=True) # May get synced by Gpl.
            ...
            with torch.autograd.profiler.record_function('Content_Aware_GAN_Compression_kd_loss'):    
                ...
                kd_style_loss = F.l1_loss(_gen_ws, _gen_teacher_ws)
                mimic_layer = [2,3,4,5]
                kd_simi_loss = torch.zeros_like(kd_style_loss)
                for i in mimic_layer:
                    f1 = student_SKD_feat_list[i-1][:batch]
                    f2 = student_SKD_feat_list[i-1][batch:]
                    s_simi = F.cosine_similarity(f1[:, None, :], f2[None, :, :], dim = 2)

                    f1 = teacher_SKD_feat_list[i-1][:batch]
                    f2 = teacher_SKD_feat_list[i-1][batch:]
                ...
                loss_Gmain = loss_Gmain + 30 * kd_simi_loss
