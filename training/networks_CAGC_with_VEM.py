...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws, return_style_scalars=False, return_rgb_list=False, **block_kwargs):
        block_ws = []
...
class Celu(nn.Module):
    def __init__(self, alpha=1.0):
        ...
        self.alpha = alpha
    def forward(self, x):
        output = torch.max(torch.zeros_like(x), x) + \
        torch.min(torch.zeros_like(x), self.alpha * (torch.exp(x/self.alpha) - 1.0))
        return output
...
class ResnetEBMBlock(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, activation, use_residual=True):
    ...
    def build_conv_block(self, in_dim, out_dim):
        ...
        #conv_layer_2 = SNConv2d(in_dim, out_dim, kernel_size=3, stride=1, padding=1, lower_bound=lower_bound)
        #conv_layer_2.weight.data.normal_(mean=0.0, std=std)
        #conv_layer_2.bias.data.fill_(0.0)
...
class ResnetEBM(nn.Module):
    ...
    def __init__(self,input_nc, n_blocks=7):
        ...
        setattr(self, 'primal_%d'%0,ResnetEBMBlock(input_nc, output_nc, activation=nn.LeakyReLU(), use_residual=use_residual))
        for i in range(1, n_blocks):
            if i == 1:
                ...
                input_nc = output_nc
                output_nc = output_nc
            setattr(self, 'primal_%d'%i,ResnetEBMBlock(input_nc, output_nc, activation=nn.LeakyReLU(), use_residual=use_residual))
